...
namespace Forge {
	namespace Graphics
	{
		...
		Void GLGraphicsContextState::BindVertexDecleration(TWeakPtr<AbstractVertexDecleration> vertex_decleration)
		{
			...
			if(m_active_vertex_decleration.m_id_num != new_id)
			{
		...
		Void GLGraphicsContextState::DeleteHardwareBuffer(TWeakPtr<AbstractHardwareBuffer> hardware_buffer)
		{
			if (hardware_buffer.IsExpired())
				FORGE_EXCEPT(ExceptionType::FORGE_INVALID_OPERATION)

			U32 id = hardware_buffer.Lock()->GetNativeHandle().m_id_num;
			U32 active_id = this->m_active_buffers[static_cast<Size>(hardware_buffer.Lock()->GetBufferType())].m_id_num;

			if (id == active_id)
				this->m_active_buffers[static_cast<Size>(hardware_buffer.Lock()->GetBufferType())].m_id_num = 0;

			FORGE_GL_CHECK_ERROR(glDeleteBuffers(1, &id))
		}
		Void GLGraphicsContextState::DeleteVertexDecleration(TWeakPtr<AbstractVertexDecleration> vertex_decleration)
		{
			if (vertex_decleration.IsExpired())
				FORGE_EXCEPT(ExceptionType::FORGE_INVALID_OPERATION)

			U32 id = vertex_decleration.Lock()->GetNativeHandle().m_id_num;

			if (id == m_active_vertex_decleration.m_id_num)
				m_active_vertex_decleration.m_id_num = 0;

			FORGE_GL_CHECK_ERROR(glDeleteVertexArrays(1, &id))
		}
	}
}
