...
namespace Forge {
	namespace Graphics
	{
		...
		public:
			/**
			 * @brief Deletes a hardware buffer and removes it from the table of
			 * currently bound hardware buffers if bound.
			 * 
			 * @throws InvalidOperationException if the TWeakPtr is expired.
			 */
			virtual Void DeleteHardwareBuffer(TWeakPtr<AbstractHardwareBuffer> hardware_buffer) = 0;

			/**
			 * @brief Deletes a vertex decleration.
			 *
			 * @throws InvalidOperationException if the TWeakPtr is expired.
			 */
			virtual Void DeleteVertexDecleration(TWeakPtr<AbstractVertexDecleration> vertex_decleration) = 0;
		};
